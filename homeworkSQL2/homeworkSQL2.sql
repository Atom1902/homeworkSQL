...
from client join department d on client.Department_idDepartment = d.idDepartment
...
select (select count(*)
        from client c join department d on c.Department_idDepartment = d.idDepartment) as AllCities,
       (select count(*)
        from client c1 join department d1 on c1.Department_idDepartment = d1.idDepartment
        where d1.DepartmentCity = 'Lviv') as CityLviv from dual;

-- 10.Знайти кредити, які мають найбільшу суму для кожного клієнта окремо.


select c.LastName, c.FirstName, max(a.Sum) as 'Max credit'
from application a join client c on c.idClient = a.Client_idClient
...
where d.DepartmentCity = 'lviv' and a.Sum > 5000;
...
select c.FirstName, c.LastName, a.Sum, a.CreditState
from client c join oin application a on c.idClient = a.Client_idClient
